...
## Eingang
- Eindeutig ermittelter Debitor (Pflicht): debitorSearch.status = unique und debitorNo
...
- Das Tool liefert in filejson ein JSON-Objekt mit Table1 als vollstaendige Referenzliste fuer genau diesen Debitor.
...
- bei unique:
  ...
  - optional articleReferenceSearch.selectedItem.customerRefNo
  - optional articleReferenceSearch.selectedItem.variantCode
- bei ambiguous:
  - articleReferenceSearch.candidates[] mit itemNo und description
  - optional customerRefNo und variantCode je Kandidat

## Regeln
- Vor dem Tool-Aufruf muss der Debitor eindeutig bestimmt sein; ohne eindeutigen Debitor kein Artikelreferenz-Matching.
...
- Table1 ist bereits auf den uebergebenen CustomerNo gefiltert.
- Filtere Table1 anschliessend nur noch mit articleReferenceText gegen CustomerRefNo.
...
- customerRefNo kommt aus CustomerRefNo.
- variantCode kommt aus VariantCode.
- Wenn CustomerRefNo leer oder unpassend ist, behandle die Zeile nicht als Treffer fuer die Anfrage.
- Wenn keine Artikelbeschreibung im Tool-Ergebnis enthalten ist, verwende als description den Wert aus CustomerRefNo.
- Optional: wenn ein internes Artikeldetail-Tool verfuegbar ist, Beschreibung je itemNo daraus anreichern.
